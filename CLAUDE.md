...
- DOM events the free SPA emits for add-ons: `websac-preset-profile-toggled` (detail `{profileId, enabled, presetId, save()}`); it listens for `websac-license-changed` and `websac-white-label-changed`.

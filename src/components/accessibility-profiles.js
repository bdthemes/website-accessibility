...
const AccessibilityProfiles = ({ value, allProfiles, onChange, accessibilityContext }) => {
    const { items } = value;
    ...
    const { currentProfile, setProfile } = accessibilityContext || {};
    ...
    const handleProfileClick = (profile) => {
        if (isFrontend && setProfile) {
            // In frontend: apply profile to context
            setProfile(profile);
            console.log('Applied profile in frontend:', profile);
        } else if (onChange) {
            // In editor: call onChange callback
            onChange(profile);
            console.log('Profile selected in editor:', profile);
        }
    };
    ...
    const profilesGrid = (
        <Row gutter={[16, 16]} className="wap-accessibility-profiles__grid">
            {selectedProfiles.map(profile => {
                const isActive = isFrontend && currentProfile?.id === profile.id;
                
                return (
                    <Col span={12} key={profile.id}>
                        <div 
                            className={`wap-accessibility-profiles__item ${isActive ? 'wap-accessibility-profiles__item--active' : ''}`}

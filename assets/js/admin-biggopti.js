jQuery(document).ready(function ($) {
    ...
    $(document).on('click', '.one-accessibility-biggopti.is-dismissible .notice-dismiss', function () {
        $this = $(this).parents('.one-accessibility-biggopti');
        var $id = $this.attr('id') || '';
        var $time = $this.attr('dismissible-time') || '';
        var $meta = $this.attr('dismissible-meta') || '';
        $.ajax({
            url: (window.OneAccessibilityBiggoptiConfig && OneAccessibilityBiggoptiConfig.ajaxurl) ? OneAccessibilityBiggoptiConfig.ajaxurl : (typeof ajaxurl !== 'undefined' ? ajaxurl : ''),
            type: 'POST',
            data: {
                action: 'one_accessibility_biggoptis',
                id: $id,
                meta: $meta,
                time: $time,

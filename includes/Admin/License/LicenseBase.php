...
if (!class_exists("LicenseBase")) {
class LicenseBase {
    public $key = "303F432E7F8FC3A2";
    private $license_key_constant = "WEBSITE_ACCESSIBILITY_LICENSE_KEY";
    private $product_id = "11";
    private $product_base = "website-accessibility-pro";
    ...
    function __construct($plugin_base_file = '') {
        ...
        if ($this->has_check_update) {
            if (function_exists("add_action")) {
                add_action('admin_post_website-accessibility-pro_fupc', function () {
                ...
                if ($this->is_theme) {
                    ...
                    add_filter('plugin_row_meta', function ($links, $plugin_file) {
                        if (plugin_basename($this->plugin_file) == $plugin_file) {
                            $links[] = " <a class='edit coption' href='" . esc_url(admin_url('admin-post.php') . '?action=website-accessibility-pro_fupc') . "'>Update Check</a>";
                        }
                        return $links;
                    }, 10, 2);
    ...
    public function theme_force_update() {
        ...
        echo wp_kses_post('<h1>' . __("Update Checking..", "website-accessibility-pro") . '</h1>');
    ...
    /**
     * Check if license key is defined as a constant
     * @return string|null Returns the license key if defined, null otherwise
     */
    private function get_license_key_from_constant() {
        if (defined($this->license_key_constant) && !empty(constant($this->license_key_constant))) {
            return constant($this->license_key_constant);
        }
        return null;
    }
    ...
    final function _check_wp_plugin($purchase_key, &$error = "", &$response_obj = null) {
        // Check if license key is defined as a constant
        $constant_key = $this->get_license_key_from_constant();
        if (!empty($constant_key)) {
            $purchase_key = $constant_key;
        }

...
class Admin_Feeds {
				...
				<div class="activity-block">
					...
					<p>
						...
						<a href="<?php echo esc_url( $feed->demo_link ); ?>" target="_blank">
							<?php esc_html_e( 'Learn more...', 'website-accessibility' ); ?>
						</a>
					</p>
				</div>
				<?php
	...
	private function get_rss_posts_data() {
		...
		if ( ! empty( $cached_data ) ) {
			...
			if ( is_wp_error( $rss ) ) {
				return '<li>' . esc_html__( 'Items Not Found', 'website-accessibility' ) . '.</li>';
			}
		...
		<div class="bdt-widget">
			<ul>
				<?php if ( empty( $rss_items ) ) : ?>
					<li><?php esc_html_e( 'Items Not Found', 'website-accessibility' ); ?>.</li>
				<?php else : ?>
						...
						<li>
							<a target="_blank" href="<?php echo esc_url( $item['link'] ); ?>"
								...
								<?php if ( $this->is_feed_item_new( $item['date'] ) ) : ?>
									<span class="bdt-feed-badge bdt-feed-badge--new"><?php esc_html_e( 'New', 'website-accessibility' ); ?></span>
								<?php endif; ?>
								<?php echo esc_html( $item['title'] ); ?>
							</a>
							<span class="bdt-date" style="display: block; margin: 0;">
								<?php echo esc_html( human_time_diff( $item['date'], current_time( 'timestamp' ) ) . ' ' . __( 'ago', 'website-accessibility' ) ); ?>
							</span>
